...
def right_circular_shift(bit, shift):
    length = len(bit) - shift
    return bit[length:] + bit[:length]
...
def schedule_right_shift(round):
    sls = [0,1,2,2,2,2,2,2,1,2,2,2,2,2,2,1]
    return sls[round]


print("Which one do you prefer?")
print("1. Enter cipher text as hexadecimal (1b 3a 4f ....)")
print("2. Enter cipher text as normal text")
...
# turn cipher text into block of 64 bits
p = ""
if choice == "2":
    for i in cipher:
        p += format(ord(i), "08b")
    ...
    print(temp)
    for i in temp:
        p += format(int(i, 16), "08b")
    cipher = p

c_block = []
for i in range(len(cipher) // 64):
    c_block.append(cipher[i * 64:(i + 1) * 64])
print("The 64 bits blocks of cipher text are :")
print(c_block)
print()
...
for index in range(len(c_block)):
    i = c_block[index]
    lkey = i_lkey
    rkey = i_rkey

    # initial permutation of 64bits block of cipher text
    block = initial_permute(i)
    print("Block No.", index + 1, "of cipher text before and after initial permutation ")
    print(i)
    print(block)
    print()

    # convert 64 bits block of cipher text into two 32 bits blocks
    lblock = block[:32]
    rblock = block[32:]
    print("\nBlock of cipher text before and after partition into left and right block")
    print(block)
    print(lblock)
    print(rblock)
    print()
    ...
    for roundNum in range(16):
        ...
        print(lkey)
        lkey = right_circular_shift(lkey, schedule_right_shift(roundNum))
        print(lkey)
        print()

        print("Right key before and after left circular shift")
        print(rkey)
        rkey = right_circular_shift(rkey, schedule_right_shift(roundNum))
        print(rkey)
        print()
        ...
        # temporary holding the right block of cipher text before it get manipulated
        temp_rblock = rblock

        # expand the 32 bits block of cipher text into 48 bits with expansion box
        ...
        # xor the right block of cipher text with the key
    ...
    # inverse initial permutation on the combined left and right block
...
for i in plain:
    wplain += i
for i in range(len(wplain) // 8):
    decplain = int(wplain[i * 8: (i + 1) * 8], 2)
    hexplain += hex(decplain)[2:] + " "
print("The plain text in hexadecimal:", hexplain)
print()

plaintext = ''
for i in range(len(wplain) // 8):
    decplain = int(wplain[i * 8: (i + 1) * 8], 2)
    plaintext += chr(decplain)
